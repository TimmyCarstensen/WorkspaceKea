...
public class Person{
    ...
    public String getMessageFromWall(int index)
    {
        String message = "";
        if(index < wall.size() && index >= 0){
            message = wall.get(index);
        }
        return message;
    }
    ...
    private boolean receiveMessage(Message message){
        ...
        if(receiver.getName().equals("Oleks") && !author.isEnemies(receiver, author)){
            addEnemy(receiver); // add friend to this person's friendlist
            receiver.addEnemy(author);
            
        } // Make friends.
        else if(message.getMessage().equals("/friend") && !isEnemies(author, receiver) && 
                !isFriends(author, receiver)){ // make friends
    ...
    public boolean isEnemies(Person author, Person receiver){
        boolean found = false;
        // Tjek the friendlist with otherPerson.
        for(int count = 0; count < enemies.size(); count++){
            // Compare this person with otherPerson.
            if(author.getEnemies().get(count).getName().equals(receiver.getName())){ 
                found = true;
                break;
            }
        }
